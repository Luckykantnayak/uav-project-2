...
class SMT:
    ...
    def init_bot_loc(self,bot_loc):
        ...
        for r in range (self.R):
            ...
            self.s.add (Or (self.X[r][self.T-1] != int (x), self.Y[r][self.T-1] != int (y)))  # The final location cannot be initial location
            self.reachable_cells.append([(x,y)])      
            self.reachable_cells2.append([(x,y)])                                           # add the initial location of bot in reachable cells
    ...
    def collect_reachables(self,r,loc,d):
        ...
        for x in range (xl, xh):
            for y in range (yl, yh):
                ...
                if (self.map[y][x] == 0.5 or self.map[y][x] == 1.0 and (abs (x - rx) + abs (y - ry) <= self.T)):  
                    self.reachable_cells2[r].append((x,y))
    ...
    ##########
    def reward(self,rbot,current_loc,next_loc):
        (nx,ny) = (int(next_loc[0]),int(next_loc[1]))
        if(self.map[ny][nx] == 0.5):
            return self.cell_age[ny][nx]*self.WR_Vis
        elif(self.map[ny][nx] == 1.0):
            return self.near(rbot,current_loc, next_loc)*self.WR_Cov
        else:
            return -1000
    ##########
    def near(self,rbot,current_loc,next_loc):
        ...
        for cell in self.visible_cells:
            ...
            for r in range(self.R): 
                ...
                if r!=rbot and   (cell not in self.reachable_cells[r]): # (abs(x-rx)+abs(y-ry)> 1): 
            ...
            for r1 in range (self.R):
                for r2 in range (r1+1,self.R):
                    if (r1 != r2):
                        ...
                        self.s.add (Or (self.X[r1][t] != self.X[r2][t], self.Y[r1][t] != self.Y[r2][t]))
        ...
        for r in range (self.R):
            for t in range (self.T):
                for obst in obst_loc:
                    ...
                    self.s.add (Or (self.X[r][t] != obst[0], self.Y[r][t] != obst[1]))
                ...
                if(t<self.T-1):
                    for curr in self.reachable_cells[r]:
                        for next in self.reachable_cells[r]:
                            cx,cy = curr
                            nx,ny = next
                            self.s.add(Implies(And(And (self.X[r][t] == int(cx), self.Y[r][t] == int(cy)),And (self.X[r][t+1] == int(nx), self.Y[r][t+1] == int(ny))),self.Re[r][t] == self.reward(r,[cx,cy],[nx,ny])))
...
def main(R,T,Dx,Dy,plots_dir,wp_dir,vis = False):
    ...
    vis_dis = 1
    cells_need_to_be_covered = Dx*Dy-len(obst_pos)-R
    cells_covered = 0
    n_neighbors = 5
    ...
    while True:
        ...
        if dt<60:
            print("Time taken for horizon {} : {} sec, Total cells covered : {} , Visible cells : {} \n".format(k,dt,cells_covered,len(visible_cells)))
        else:
            print("Time taken for horizon {} : {} min, Total cells covered : {} , Visible cells : {} \n".format(k,dt/60,cells_covered,len(visible_cells)))
